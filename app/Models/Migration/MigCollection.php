...
class MigCollection
{
    ...
    public function readAll() {
        ...
        $structData = DB::select("SELECT * FROM {$migPubDb}.NAV_STRUCTURE WHERE ID = {$this->id}");
        $this->navStruct = [];
        foreach($structData[0] as $key => $val) {
            $this->navStruct[$key] = $val;
        }

        $gtSlData = DB::select("SELECT * FROM {$migPubDb}.NAV_GLAVNA_TABELA WHERE ID = {$this->id} AND LANGUAGE = 'sl'");
        $this->navGT_sl = [];
        foreach($gtSlData[0] as $key => $val) {
            $this->navGT_sl[$key] = $val;
        }

        $gtEnData = DB::select("SELECT * FROM {$migPubDb}.NAV_GLAVNA_TABELA WHERE ID = {$this->id} AND LANGUAGE = 'en'");
        $this->navGT_en = [];
        foreach($gtEnData[0] as $key => $val) {
            $this->navGT_en[$key] = $val;
        }

        //print_r($this->navStruct);
        //print_r($this->navGT_sl);
        //print_r($this->navGT_en);

        // $this->navStruct["METAREF_PUB_ID"]


        // Get children menus
        $childData = DB::select("SELECT * FROM {$migPubDb}.NAV_STRUCTURE WHERE PARENT_ID = {$this->id}");
        $this->children = [];
        ...
        // Get children pubs
        $pubsData = DB::select("SELECT * FROM {$migPubDb}.PUB_GLAVNA_TABELA WHERE MENU_ID = {$this->id}");
        $this->pubs = [];
        foreach($pubsData as $rowIdx => $pub) {
            $tableRowArr = [];
            foreach ($pub as $cKey => $cVal) $tableRowArr[$cKey] = $cVal;
            $this->pubs[$rowIdx] = $tableRowArr;
        }

        // Find primary
        $parentId = $this->navStruct["PARENT_ID"];
        if ($parentId) {
            while (true) {
                $parentData = DB::select("SELECT * FROM {$migPubDb}.NAV_STRUCTURE WHERE ID = {$parentId}");
                $nextParent = $parentData[0]->PARENT_ID;
                if (!$nextParent) {
                    $this->primary = [];
    ...
    public function makeEntity() {
        ...
        $this->entity->handle_id = "menu".$this->id;

        $this->entity->parent = $this->navStruct["PARENT_ID"] ? "menu".$this->navStruct["PARENT_ID"] : "";
        $this->entity->primary = $this->primary ? "menu".$this->primary["ID"] : "";

        $this->entity->struct_type = $struct_type;

        $hasParent = $this->navStruct["PARENT_ID"] ? true : false;
    ...
    public function makeMetsXml() {
        return $this->makeMetsXml_noMetaRef();
    }


    private function makeMetsXml_noMetaRef() {
        ...
        $menuId = "menu".$this->id;
        $type = "collection";
        $timestamp = "2018-02-28T00:00:00";

        $titleSl = MigUtil::wrapCDATA($this->navGT_sl["TITLE"]);
        $titleEn = MigUtil::wrapCDATA($this->navGT_en["TITLE"]);

        $descSl = MigUtil::wrapCDATA($this->navGT_sl["CONTENT"]);
        $descEn = MigUtil::wrapCDATA($this->navGT_en["CONTENT"]);

        $publisher = "Inštitut za novejšo zgodovino";

        $searchResultsSort = $this->navGT_sl["SORT"];
        $searchResultsShow = $this->navGT_sl["DISPLAY"];

        $parentId = $this->navStruct["PARENT_ID"] ? "menu".$this->navStruct["PARENT_ID"] : "";
...
<METS:mets xmlns:METS="http://www.loc.gov/METS/"
    ...
    ID="{$menuId}"
    TYPE="{$type}"
    OBJID="http://hdl.handle.net/11686/{$menuId}"
    ...
    http://www.loc.gov/standards/premis/v1 https://www.loc.gov/standards/premis/premis.xsd">
    ...
    <METS:dmdSec ID="default.dc">
        <METS:mdWrap MDTYPE="DC" MIMETYPE="text/xml">
            <METS:xmlData>
                <dcterms:title xml:lang="slv">{$titleSl}</dcterms:title>
                <dcterms:title xml:lang="eng">{$titleEn}</dcterms:title>
                <dcterms:description xml:lang="slv">{$descSl}</dcterms:description>
                <dcterms:description xml:lang="eng">{$descEn}</dcterms:description>
    ...
    <METS:amdSec ID="default.amd">
        ...
        <METS:techMD ID="default.si4">
            <METS:mdWrap MDTYPE="OTHER" OTHERMDTYPE="COLLECTION" MIMETYPE="text/xml">
                <METS:xmlData xmlns:collection="http://sistory.si/schema/si4/collection" xsi:schemaLocation="http://sistory.si/si4/collection ../../schema/collection.1.0.xsd">
                    <collection:description xml:lang="slv">{$descSl}</collection:description>
                    <collection:description xml:lang="eng">{$descEn}</collection:description>
                    <collection:externalCollection></collection:externalCollection>
                    <collection:wholeContent>false</collection:wholeContent>
                    <collection:searchResultsSort>{$searchResultsSort}</collection:searchResultsSort>
                    <collection:searchResultsShow>{$searchResultsShow}</collection:searchResultsShow>
                </METS:xmlData>
            </METS:mdWrap>
        </METS:techMD>
    </METS:amdSec>
HERE;
        ...
        $xmlComps[] = <<<HERE
    <METS:structMap ID="default.structure" TYPE="{$this->entity->entity_type}" xmlns:xlink="http://www.w3.org/1999/xlink">
        <METS:div TYPE="collection">
HERE;
        if ($parentId) {
            $xmlComps[] = <<<HERE
            <METS:mptr LOCTYPE="HANDLE" xlink:href="http://hdl.handle.net/11686/{$parentId}"/>
HERE;
        }
        $xmlComps[] = <<<HERE
...
HERE;
        ...
        // Child collections
        foreach ($this->children as $childCol) {
            $childId = "menu".$childCol["ID"];
            $xmlComps[] = <<<HERE
                <METS:div TYPE="collection">
...
HERE;
    ...
    <METS:behaviorSec ID="default.behavior" xmlns:xlink="http://www.w3.org/1999/xlink">
        <METS:behavior BTYPE="collection">
            <!-- URL prelje do XML sheme, s katero se parsa mets dokumente za ElasticSearch -->
...
HERE;
    ...
    */
    ...
    public function makeEntity() {
        ...
        $this->entity->parent = $this->getData("PUB_GLAVNA_TABELA")[0]["RELATION"] ?
            $this->getData("PUB_GLAVNA_TABELA")[0]["RELATION"] : 0;
        $this->entity->primary = $this->primary ? $this->primary["ID"] : 0;
